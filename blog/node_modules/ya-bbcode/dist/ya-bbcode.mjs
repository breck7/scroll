...
var yabbcode = class {
	...
	#contentLoop(tagsMap, content) {
		...
		for (const tag of tagsMap) {
			...
			if (!this.contentModules[module.type]) throw new Error("Cannot parse content block. Invalid block type [" + module.type + "] provided for tag [" + tag.module + "]");
			if (module.type === "replace") content = this.contentModules.replace(tag, module, content);
			else if (module.type === "content") content = this.contentModules.content(tag, module, content);
			else if (module.type === "ignore") content = this.contentModules.ignore(tag, module, content);
			if (tag.children.length > 0 && module.type !== "ignore") content = this.#contentLoop(tag.children, content);
		}
		return content;
	}
...
//#endregion
export { ya_bbcode_default as default };
//# sourceMappingURL=ya-bbcode.mjs.map